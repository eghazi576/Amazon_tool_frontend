...
const categories = [
  {
    ...
    faqs: [
      ...
      },
      {
        ...
        a: "Jungle Scout and Helium 10 are broad Amazon seller suites covering keyword research, listing optimization, PPC, and more. WholesaleOS is laser-focused on product and brand viability scoring. Its Brand Intelligence module scores brands across 11 criteria — including IP complaint history, hazmat catalog percentage, MAP enforcement, and FBA seller count — a workflow that general tools do not offer. WholesaleOS is also built on live Amazon market data, giving you 90-day price history, BSR trends, and real-time FBA seller counts.",
      },
      {
        q: "What data does WholesaleOS use?",
        a: "WholesaleOS uses live Amazon marketplace data, including price history, Best Seller Rank, review counts, FBA/FBM offer counts, and Amazon's own 'Bought in past month' sales badge. For brand vetting, it also factors in IP complaint history and business and domain registration checks.",
      },
      {
        q: "Is WholesaleOS free to use?",
        a: "Access is approved by an administrator. There is no separate sign-up — sign in with your email and password, and a new email creates an account that an admin reviews. Once approved, you can start researching products and evaluating brands.",
      },
    ],
  },
  {
    ...
    faqs: [
      ...
      },
      {
        ...
        a: "WholesaleOS uses a three-step cascade. First, it reads Amazon's own 'Bought in past month' badge — the most accurate signal available. If that is not available, it counts actual BSR drop events in the 30-day series (each drop typically represents a sale). As a last resort it extrapolates from the 90-day BSR average using a category-specific BSR-to-sales lookup table.",
      },
      {
    ...
    faqs: [
      {
        ...
        a: "Sales estimates are approximations, not guarantees. The most accurate source is Amazon's own 'Bought in past month' badge, which reflects real purchases in the last 30 days. When that is unavailable, WholesaleOS counts actual BSR drops in the 30-day data series — each drop generally corresponds to at least one sale. All estimates carry inherent uncertainty; use them as directional signals alongside BSR trend and review velocity.",
      },
      {
        q: "How current is the price and BSR data?",
        a: "WholesaleOS tracks Amazon prices and BSR in near real-time, with data typically refreshed every 1–6 hours depending on the product's sales velocity. When you analyze an ASIN, it fetches the most current snapshot available at that moment, plus the full 90-day historical series.",
      },
      {
        q: "Why might some fields show no data?",
        a: "Some products — especially newly launched ASINs or products with very low sales velocity — do not have enough historical data for all fields to populate. In those cases, WholesaleOS shows null rather than an incorrect value, and falls back to alternative calculation methods where possible. A missing data field is useful information in itself: it often signals a product too new or too inactive to confidently evaluate.",
      },
    ],
  },
  {
